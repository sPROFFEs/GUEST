...
A modular VPN + firewall gateway you snapshot once and clone forever. Each instance is a small Debian 12/13 (or Ubuntu Server 22.04/24.04) VM that:
...
- **Debian 12** (Bookworm) or **Debian 13** (Trixie) minimal install, or **Ubuntu Server 22.04 / 24.04 / 26.04**. The installer detects the network backend (ifupdown vs netplan) automatically.
